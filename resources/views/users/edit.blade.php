    ...
    <form action="{{ route('dashboard.users.update') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="user_group_id" class="form-control">
                <option>Please choose group</option>
                @if(!empty($listGroup))
                    @foreach($listGroup as $group)
                        <option
                            value="{{ $group->group_id }}" {{ old('user_group_id') == $group->group_id || $user->user_group_id == $group->group_id ? 'selected' : false }}>{{ $group->group_name }}</option>
                    @endforeach
                @endif
            </select>
        ...
        <div class="form-group">
            ...
            <select name="user_status" class="form-control">
                ...
                <option value="1" {{ old('user_status') === 1 || $user->user_status === 1 ? 'selected' : false }}>Active</option>
                <option value="0" {{ old('user_status') === 0 || $user->user_status === 0 ? 'selected' : false }}>InActive</option>
            </select>
